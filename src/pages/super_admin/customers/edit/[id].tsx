...
const EditPage = () => {
  ...
  const {data} = useGetCustomerRequestDetailsQuery(id, {skip: !id});
  ...
  const editHandle = async (values: any) => {
    ...
    try {
      const res = await updateCustomerRequest({id, body: values}).unwrap();
      //   console.log(res);
      if (res?.id) {
  ...
  return (
    <div>
      ...
      <div style={{border: "1px solid #ccc", padding: "20px"}}>
        ...
        <h3 className="text-lg">
          <span className="font-medium">MC/FF Number:</span> {mcNumber} lbs
        </h3>
        <h3 className="text-lg">
